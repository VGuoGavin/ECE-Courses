...
ITAEPoles = 1.5*[-0.7081; -0.5210+1.068*1i; -0.5210-1.068*1i];
...
grid on
%%
BesselPoles = 1.5*[-0.9420; -0.7455+0.7112*1i;  -0.7455-0.7112*1i];
...
SODPoles = [-2; -3.6+2.7*1i; -3.6-2.7*1i];
...
r_gain = 1/dcgain(sys_ag_LQR);
% 为什么这里的DC gain刚好为1

[num, den] = ss2tf(Fbar-Gu * K_LQR, Gr, Hbar, 0);
sys_tf = tf(num,den)
...
step(sys_ag_Disturb)
grid on
sys_ag_Disturb_Control = ss(Fbar - Gu*K_LQR, Gv, -K_LQR, 0);  %这个是输入对噪声的响应吗？
figure(23) 
step(sys_ag_Disturb_Control)
grid on

%Provide plots showing the response of the output, y, to a unit step in u.

%plot u with disturbance
sys_u_Disturb = ss(Fbar-Gu*K_LQR, Gv, [0,1,0], 0);
figure(24)
step(sys_u_Disturb)
