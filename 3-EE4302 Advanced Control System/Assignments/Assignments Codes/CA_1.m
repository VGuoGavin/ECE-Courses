...
A_Plant = [0,       1;
                -1.2,  -3.71]; % Process Matrix
B_Plant = [0; 1]; % Input Matrix
C_Plant_State1 = [1,0; 0,0]; % Output Matrix
C_Plant_State2 = [1, 0]; % Output Matrix Assuming x2 is the Output
sys1 = ss(A_Plant, B_Plant, C_Plant_State2, 0); % Use function 'ss' to establish state-space model
...
Poles = eig(A_Plant); % The open-loop poles are the eigenvalue of the original plant process matrix
...
% 计算是否 controllable
ControllabilityMatrix = [B_Plant, A_Plant*B_Plant];
...
%% 随便给个K 试一试
K_SimpleExp = [-0.6122 + 2.285*1i, -0.6122 - 2.285*1i]; 
Signal_KX_SimpleExp = ss(0,[0,0],0 ,K_SimpleExp); % Dummy system representing the value of K*x
sys_fb_SimpleExp = feedback(sys1, Signal_KX_SimpleExp,-1); % Form a feedback system.
...
F_ClosedLoop = A_Plant-B_Plant*K_SimpleExp;
...
sys1 = ss(A_Plant, B_Plant, C_Plant_State1, 0); 
Second_ref_poles = [-3.2150 - 2.4113i;  -3.2150 + 2.4113i];
Second_ref = acker(A_Plant, B_Plant, Second_ref_poles); 
Signal_KX_ITAE = ss(0,  [0,0],  0,  Second_ref);
sys_fb_ITAE = feedback(sys1,  Signal_KX_ITAE,  -1);
...
% other calculations.
sys_cl_2st_r = Ks_2st * sys_fb_ITAE; 
...
disp("Second-order reference model Design Poles scaling gain:")
disp(Ks_2st)
...
ITAEPoles = 3.5*[-0.7071+0.7071*1i;-0.7071-0.7071*1i]; % ITAE Design Poles as a column vector
% In MATLAB, 'a+b*1i' is common way of defining complex value
K_ITAE = acker(A_Plant,B_Plant,ITAEPoles); 
% Use function 'acker' to calculate the controller gain using Ackermann's Formula
Signal_KX_ITAE = ss(0,[0,0],0,K_ITAE);
sys_fb_ITAE = feedback(sys1, Signal_KX_ITAE,-1);

% 这里查看一下反馈系统的传输函数方便求DC增益
[b,a] = ss2tf(sys_fb_ITAE.A, sys_fb_ITAE.B, sys_fb_ITAE.C, sys_fb_ITAE.D);
tf_ITAE = tf(b(1,:),a);
disp("Feedback trasfer function:")
disp(tf_ITAE)
...
BesselPoles = 3.5*[-0.8660+0.5000*1i; -0.8660-0.5000*1i];
K_Bessel = acker(A_Plant,B_Plant,BesselPoles);
Signal_KX_Bessel = ss(0,[0,0],0,K_Bessel);
sys_fb_Bessel = feedback(sys1,Signal_KX_Bessel,-1);
...
K_SOD = acker(A_Plant,B_Plant,SODPoles);
Signal_KX_SOD = ss(0,[0,0],0,K_SOD);
sys_fb_SOD = feedback(sys1,Signal_KX_SOD,-1);
...
Q_LQR = [100, 0; 0, 100]; % Define penalty matrix Q
...
sys_fb_LQR = feedback(sys1,Signal_KX_LQR,-1);
